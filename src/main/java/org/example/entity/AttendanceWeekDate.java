...
public class AttendanceWeekDate {
    ...
    @Builder
    public AttendanceWeekDate(ATTENDANCE_STATE monday, ATTENDANCE_STATE tuesday, ATTENDANCE_STATE wednesday, ATTENDANCE_STATE thursday, ATTENDANCE_STATE friday) {
        this.monday = monday;
    ...
    protected void onCreate() {
        ...
        ZonedDateTime friday = now.with(DayOfWeek.FRIDAY); // 현재 주의 금요일을 가져옵니다.

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");

        this.mondayDate = LocalDate.parse(monday.format(formatter)); // "yyyy-MM-dd" 형식으로 포맷팅
        this.fridayDate = LocalDate.parse(friday.format(formatter)); // "yyyy-MM-dd" 형식으로 포맷팅
